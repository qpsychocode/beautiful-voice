...
STRINGS = {
    ...
    "set_live_hint": "Nemotron распознаёт речь прямо по ходу, поэтому текст готов примерно через полсекунды после остановки. Остальные модели распознают законченные фразы во время записи.",
    ...
    "set_mirror_hint": "«Самый быстрый» за пару секунд проверяет оба источника и качает с того, что быстрее. GitHub — наша копия моделей, работает там, где Hugging Face недоступен.",
    ...
    "source_auto": "Самый быстрый",
    "update_available": "Вышла версия {version}",
    "update_download": "Скачать обновление",
    "update_open": "Открыть страницу загрузки",
    "update_install": "Установить и перезапустить",
    "update_downloading": "Скачиваю… {pct}%",
    "update_retry": "Попробовать снова",
    "update_failed": "Не удалось скачать обновление: {error}",
    "update_check": "Проверить обновления",
    "update_checking": "Проверяю…",
    "update_latest": "У вас последняя версия.",
    "set_autostart_mac": "Запускать при входе в систему",
    "blurb_nemotron-3.5-asr-streaming-full": "Тот же Nemotron 3.5 без 8-битного сжатия: точнее всех на живой диктовке, но весит больше и работает чуть медленнее.",
    "set_caption": "Показывать текст, пока я говорю",
    "set_caption_hint": "Распознанные слова появляются над плашкой записи прямо по ходу речи. Работает с моделями Nemotron.",
}
